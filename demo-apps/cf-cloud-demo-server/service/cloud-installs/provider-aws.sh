...
get_ami() {
   echo "AMI: " $(aws ec2 describe-images --filters "Name=name,Values=ubuntu/*20.04*amd64*server*" --owner amazon --query "sort_by(Images, &CreationDate)[-1].Name")
   ami_id=$(aws ec2 describe-images --filters "Name=name,Values=ubuntu/*20.04*amd64*server*" --owner amazon --query "sort_by(Images, &CreationDate)[-1].ImageId" --output text)
   echo "ami-id: ${ami_id}"
}
