...
[Eclipse Release Repository](https://repo.eclipse.org/content/repositories/californium-releases/org/eclipse/californium/cf-simplefile-server/3.3.0/cf-simplefile-server-3.3.0.jar)
...
```sh
java -jar cf-simplefile-server-3.3.0.jar.
```
...
```sh
java -jar cf-simplefile-server-3.3.0.jar [--file-root=<file-root-directory>] [--path-root=<coap-root>]
```
...
```sh
java -jar cf-helloworld-client-3.3.0.jar GETClient coap://localhost/data/file.bin file2save.bin
```
...
```
java -jar cf-simplefile-server-3.3.0.jar --file-root=/home/cali/data --path-root=files --no-tcp
...
```
java -jar cf-helloworld-client-3.3.0.jar GETClient coap://localhost/files/cf_64.png picture2save.png
java -jar cf-helloworld-client-3.3.0.jar GETClient coap://localhost/files/fw/device.hex firmware.hex
```

Access files out of the file system sub-tree results in an error response.
 
```
java -jar cf-helloworld-client-3.3.0.jar GETClient coap://localhost/files/../../other/top-secret.txt steal.txt
