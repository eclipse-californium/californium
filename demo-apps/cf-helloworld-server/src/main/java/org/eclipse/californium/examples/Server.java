...
public class Server {
	...
	private static final String HELLO_WORLD = HelloWorldServer.class.getSimpleName();
	private static final String MULTICAST = MulticastTestServer.class.getSimpleName();
	...
	public static void main(String[] args) throws IOException, ConnectorException, InterruptedException {
		...
		if (start != null) {
			...
			if (HELLO_WORLD.equals(start)) {
				HelloWorldServer.main(args2);
				return;
			} else if (MULTICAST.equals(start)) {
				MulticastTestServer.main(args2);
				return;
			}
		}
		...
		System.out.println();
		System.out.println("Usage: " + Server.class.getSimpleName() + " (" + HELLO_WORLD + "|" + MULTICAST + ")");
		if (start != null) {
