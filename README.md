...
We are publishing Californium's artifacts for milestones and releases to [Maven Central](https://search.maven.org/search?q=g:org.eclipse.californium%20a:parent%20v:3.1.0).
...
```
You can then simply depend on `3.2.0-SNAPSHOT`.
...
It is an instance of the [cf-plugtest-server](https://repo.eclipse.org/content/repositories/californium-releases/org/eclipse/californium/cf-plugtest-server/3.1.0/cf-plugtest-server-3.1.0.jar) from the demo-apps.
...
`java -DCOAP_NETWORK_INTERFACES='.*wpan0' -jar target/cf-helloworld-server-3.1.0.jar MulticastTestServer`
