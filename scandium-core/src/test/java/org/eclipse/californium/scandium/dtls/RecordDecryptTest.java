...
public class RecordDecryptTest {
	...
	public void setUp() throws Exception {
		...
		SecretKey encKey = new SecretKeySpec(createBytes(secureRandom, encKeyLength), "AES");
		SecretKey macKey = macKeyLength == 0 ? null
				: new SecretKeySpec(createBytes(secureRandom, macKeyLength), "AES");
		IvParameterSpec iv = new IvParameterSpec(createBytes(secureRandom, ivLength));
		payloadData = createBytes(secureRandom, payloadLength);

		session = new DTLSSession(new InetSocketAddress(InetAddress.getLoopbackAddress(), 7001), true);
	...
	private void testEncryptDecrypt(byte[] payload) throws GeneralSecurityException, HandshakeException {
		Record record = new Record(ContentType.APPLICATION_DATA, EPOCH, 1,
				new ApplicationMessage(payload, session.getPeer()), session);
		byte[] raw = record.toByteArray();
		List<Record> list = Record.fromByteArray(raw, session.getPeer());
	...
	private void testEncryptDecryptRecordFailure(byte[] payload, Juggler juggler)
		...
		dumpDiff(raw, jraw);
		List<Record> list = Record.fromByteArray(jraw, session.getPeer());
		for (Record recv : list) {
			recv.setSession(session);
			recv.getFragment();
		}
	 ...
	 */
	public static byte[] createBytes(SecureRandom generator, int size) {
