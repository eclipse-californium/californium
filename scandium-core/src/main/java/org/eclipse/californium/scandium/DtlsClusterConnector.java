...
public class DtlsClusterConnector extends DTLSConnector {
	...
	public DtlsClusterConnector(DtlsConnectorConfig configuration, DtlsClusterConnectorConfig clusterConfiguration,
		...
		this(configuration, clusterConfiguration, sessionCache, true);
	...
	protected DtlsClusterConnector(DtlsConnectorConfig configuration, DtlsClusterConnectorConfig clusterConfiguration,
		...
		this(configuration, clusterConfiguration, sessionCache, false);
	}
	 ...
	 */
	private DtlsClusterConnector(DtlsConnectorConfig configuration, DtlsClusterConnectorConfig clusterConfiguration,
					...
					if (getNodeID() != incomingNodeId) {
						...
						if (clusterNode != null) {
							...
							try {
								...
								LOGGER.info("cluster-node {}: send error:", getNodeID(), e);
							}
						} else {
							LOGGER.debug(
	...
	private DatagramPacket encode(byte direction, DatagramPacket packet, byte[] recordBytes) {
		...
		int headerLength = address.length + CLUSTER_ADDRESS_OFFSET + getClusterMacLength();
		byte[] data = packet.getData();
		int offset;
		int length;
		if (recordBytes == null) {
	...
	private DatagramPacket decode(DatagramPacket packet) {
		...
		int port = (data[offset + CLUSTER_PORT_OFFSET] & 0xff) | ((data[offset + CLUSTER_PORT_OFFSET + 1] & 0xff) << 8);
		byte[] address = Arrays.copyOfRange(data, offset + CLUSTER_ADDRESS_OFFSET,
				offset + CLUSTER_ADDRESS_OFFSET + addressLength);
		int headerLength = addressLength + CLUSTER_ADDRESS_OFFSET + getClusterMacLength();
		try {
