...
public final class ECDHECryptography {
	...
	/**
	 * Creates an ephemeral key ECDH keypair for a given curve.
	 * 
	 * This constructor is usually invoked by the server.
	 * 
	 * @param supportedGroupId
	 *            the ID of the supported group to use as defined in
	 ...
	 * @throws IllegalArgumentException if the IANA registry does not contain
	 *            a supported group with the given ID or the domain params for the
	 *            given supported group are unknown (yet)
	 */
	public ECDHECryptography(int supportedGroupId) throws GeneralSecurityException {
		// create ephemeral key pair
		SupportedGroup group = SupportedGroup.fromId(supportedGroupId);
		if (group == null) {
			throw new IllegalArgumentException(
					String.format("No Supported Group found with id %d", supportedGroupId));
		} else {
			ECParameterSpec params = group.getEcParams();
			if (params == null) {
				throw new IllegalArgumentException(
						String.format("No domain params found for supported group %s", group.name()));
			} else {
				createKeys(params);
			}
		}
	 ...
	 *            the keys
	 * @return the object containing the keys or <code>null</code> if the given
	 *            id is not supported
	 */
	public static ECDHECryptography fromNamedCurveId(int supportedGroupId) {
		...
		if (group == null) {
			...
			if (params == null) {
				...
				try {
					return new ECDHECryptography(params);
				} catch (GeneralSecurityException e) {
					// this should not happen because we are using domain parameters
					// contained in the SupportedGroup enum
					LOGGER.log(
						Level.WARNING,
						"Cannot use domain parameters of supported group [{0}] to create ephemeral keys: {0}",
	...
	private void createKeys(ECParameterSpec params) throws GeneralSecurityException {
	 ...
	 * <a href=""http://www.ecc-brainpool.org/download/Domain-parameters.pdf>
	 * ECC Brainpool Standard Curves and Curve Generation</a>.
	 * 
	...
	public enum SupportedGroup {
		...
		secp256k1(22,
				...
				1),
		secp256r1(23, // [NIST P-256, X9.62 prime256v1]
		...
		private SupportedGroup(int code, String p, String a, String b, String x, String y, String n, int h) {
			this(code);
			...
			this.params = new ECParameterSpec(curve, g, new BigInteger(n, 16), h);
		}
