...
public class ConnectorTestUtil {
	...
	public static final String NO_TRUST_NAME = "client-1";
	...
	public static SSLTestContext initializeNoTrustContext(String aliasCredentials, String aliasTrusts) throws Exception {
		KeyManager[] keys = SslContextUtil.loadKeyManager(SslContextUtil.CLASSPATH_PROTOCOL + KEY_STORE_LOCATION,
				aliasCredentials, KEY_STORE_PASSWORD, KEY_STORE_PASSWORD);
		TrustManager[] trusts = SslContextUtil.loadTrustManager(SslContextUtil.CLASSPATH_PROTOCOL
				+ NO_TRUST_STORE_LOCATION, aliasTrusts, TRUST_STORE_PASSWORD);

		Principal subjectDN = getSubjectDN(keys, aliasCredentials);
		log("keys ", keys);
		log("trusts", trusts, true);

		keys = SslContextLoggingUtil.logging(keys, aliasCredentials);
		trusts = SslContextLoggingUtil.logging(trusts, aliasCredentials + "-no-trust");
