...
public class DatagramFramer extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        while (in.readableBytes() > 0) {
            ...
            ByteBuf byteBuf = in.readBytes(coapHeaderSize + bodyLength);
            // This is TCP connector, so we know this is InetSocketAddress.
            InetSocketAddress socketAddress = (InetSocketAddress) ctx.channel().remoteAddress();
            RawData rawData = new RawData(byteBuf.array(), socketAddress);
            out.add(rawData);
        }
    }
