 ...
 * </ul>
 * <p>
 * This implementation uses a <code>java.util.HashMap</code> as its backing
 * store. In addition to that the cache keeps a doubly-linked list of the
 * entries in access-time order.
 * </p>
 * <p>
 * Access to the cache's entries (e.g. <em>put</em>, <em>get</em>,
 * <em>remove</em>) is <em>not</em> thread safe. Clients should explicitly
 * serialize access to the cache from multiple threads.
 * </p>
 * <p>
...
public class LeastRecentlyUsedCache<K, V> {
	 ...
	 * @return <code>true</code> if the entry could be added to the cache,
	 *         <code>false</code> otherwise, e.g. because the cache's remaining
	 ...
	 * 
	 * If the cache contains the key but the value is <em>stale</em> the entry
	 ...
	 *         given predicate or <code>null</code> if no value matches
	 */
