	...
	public static byte[] serializeConfidentialData(OptionSet options, byte[] payload, int realCode) {
		if (options != null) {
			...
			if (realCode > 0) {
				...
				LOGGER.error(Error.COAP_CODE_INVALID);
				throw new IllegalArgumentException(Error.COAP_CODE_INVALID);
			}
		} else {
			LOGGER.error(Error.OPTIONSET_NULL);
			throw new NullPointerException(Error.OPTIONSET_NULL);
		}
	}
	...
	public static byte[] serializeSendResponseAAD(int version, OSCoreCtx ctx, OptionSet options, boolean newPartialIV) {
		if (version == CoAP.VERSION) {
			if (ctx != null) {
				...
				throw new NullPointerException(Error.CTX_NULL);
			}
		} else {
			LOGGER.error(Error.WRONG_VERSION_NBR);
			throw new IllegalArgumentException(Error.WRONG_VERSION_NBR);
		}
	}
	...
	public static byte[] serializeReceiveResponseAAD(int version, int seq, OSCoreCtx ctx, OptionSet options) {
		if (version == CoAP.VERSION) {
			if (seq > -1) {
				...
				LOGGER.error(Error.SEQ_NBR_INVALID);
				throw new IllegalArgumentException(Error.SEQ_NBR_INVALID);
			}
		} else {
			LOGGER.error(Error.WRONG_VERSION_NBR);
			throw new IllegalArgumentException(Error.WRONG_VERSION_NBR);
			...
			if (senderID != null) {
				if (commonIV != null) {
					if (nonceLength > 0) {
						...
						LOGGER.error(Error.NONCE_LENGTH_INVALID);
						throw new IllegalArgumentException(Error.NONCE_LENGTH_INVALID);
					}
				} else {
					LOGGER.error(Error.COMMON_IV_NULL);
					throw new NullPointerException(Error.COMMON_IV_NULL);
				}
			} else {
				LOGGER.error(Error.SENDER_ID_NULL);
				throw new NullPointerException(Error.SENDER_ID_NULL);
			}
		} else {
			LOGGER.error(Error.PARTIAL_IV_NULL);
			throw new NullPointerException(Error.PARTIAL_IV_NULL);
		}
	}
