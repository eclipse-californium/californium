	...
	protected static byte[] decryptAndDecode(Encrypt0Message enc, Message message, OSCoreCtx ctx, Integer seqByToken)
		...
		if (isRequest) {
			...
			if (tmp == null) {
				...
				throw new OSException(Error.DECRYPTION_FAILED);
			} else {
		...
		try {
			...
			LOGGER.error(Error.DECRYPTION_FAILED + " " + e.getMessage());
			throw new OSException(Error.DECRYPTION_FAILED + " " + e.getMessage());
		}
	...
	private static void decodeObjectSecurity(Message message, Encrypt0Message enc) throws OSException {
		...
		if (n > 0) {
			try {
				...
				throw new OSException(Error.FAILED_TO_DECODE_COSE);
			}
		}
