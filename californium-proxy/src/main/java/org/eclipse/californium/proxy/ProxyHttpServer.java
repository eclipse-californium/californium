...
public class ProxyHttpServer {
	...
	public ProxyHttpServer(CoapServer server) throws IOException {
		this(new HttpStack(server.getConfig(), server.getConfig().getInt(NetworkConfig.Keys.HTTP_PORT), false, true));
	...
	public ProxyHttpServer(int httpPort, boolean proxy, boolean local) throws IOException {
		this(new HttpStack(httpPort,proxy,local));
	}

	public ProxyHttpServer(NetworkConfig config, int httpPort, boolean proxy, boolean local) throws IOException {
		this(new HttpStack(config,httpPort,proxy,local));
	}
